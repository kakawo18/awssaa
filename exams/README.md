...
1. 1セット8〜15問・目安15〜25分（セット33の模擬試験は65問・130分）。設問を読んで**自分の答えを決めてから**「解答と解説」を開きます。
...
| [セット33](./set-33.md) | **模擬試験①（本番形式）** | 65 | 本番と同じ65問・分野比率（20／17／15／13問）・複数選択5問。ポリシーの読解（Principal・最小権限・SCPは上限）／数値の比較（Lambdaの料金・Auto Scalingの終了順）／KMSキーの無効化／Cognitoオーソライザー／WAF／Access Analyzer／SQSの優先度／SNS FIFO／部分的なバッチレスポンス／Saga／ウォームスタンバイ／Vault Lock／EFSのエラスティック／S3 Express One Zone／GSIとLSI／Origin Shield／EFA／同時実行スケーリング／SnapStart／停止中の料金／未完了のマルチパート／ECRのエンドポイント／Budgetsのアクション／リクエスタ支払い ほか |

次に作るのは **セット34** です（実戦演習⑨、または模擬試験②）。
...
| **模擬試験** | **33** | 本番と同じ**65問・130分**・分野比率で通して解く。公式模擬試験に多い「ポリシーを読む」「数値を比べる」「動くが条件で劣る選択肢を外す」問われ方を多めにしている。途中で解説を開かず、最後にまとめて答え合わせをする |
...
- **正解記号は A〜D に均等に散らす**（現在 A 28%／B 25%／C 24%／D 24%）。記号を見ただけで当てられる問題集にしないため、
