...
- **C**：Pinpointはキャンペーン配信やセグメント配信の基盤で、プッシュ通知は今回の要件と異なります（メール送信自体はSESの仕組みを使います）。なおPinpointは2026年10月30日にサポート終了予定です。
- **D**：WorkMailは社内向けのメールボックスサービスです（2026年4月30日に新規顧客の受付を終了）。
...
**決め手**：**リフト＆シフト＝MGN**（ブロックレベルの継続的複製でダウンタイムを最小化）、**移行前の資産調査＝Application Discovery Service**。移行の定型セットです。※Application Discovery Serviceは2025年11月7日に新規顧客の受付を終了し、後継はAWS Transformです。試験では今もこの組み合わせで出るため、対応関係として押さえます。
...
- **E.** AWS Budgets で予算と予測超過のしきい値を設定し、アラートを受け取る
